...
namespace AtelierXNA
{
    public class GrilleUniverselle : GameComponent, IModele3d
    {
            ...
            for (int x = 0; x < Couleurs.GetLength(0); ++x)
            {
                for (int z = 0; z < Couleurs.GetLength(1); ++z)
                {
                    ...
                    else if (Couleurs[x, z] == SUPPORT_BAZOOKA)
                    {
                        ...
                        //ManagerModËle.AjouterModele(supportFusil);
                    }
        ...
        public Node NodePositionMonde(Vector3 positionMonde)
        {
            ...
            int x = (int)Math.Round((TailleMondeGrille.X) * pourcentageX);//(int)Math.Round((TailleMondeGrille.X - 1) * pourcentageX);
            int z = (int)Math.Round((TailleMondeGrille.Z) * pourcentageZ); //(int)Math.Round((TailleMondeGrille.Z - 1) * pourcentageZ);
            return GetNode(x, z);
        }
