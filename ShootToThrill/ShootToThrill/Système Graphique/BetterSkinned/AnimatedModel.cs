...
namespace AtelierXNA
{
    ...
    /// </summary>
    public class AnimatedModel : MObjetDeBase, IModele3d
    {
        #region Fields
       ...
       // }
        ...
        public AnimatedModel(Game game, string assetName, Vector3 positionInitiale)
           :base(game, assetName, 1,Vector3.Zero, positionInitiale)
        {
        ...
        protected override void LoadContent()
        {
           ...
           modelExtra = Modèle.Tag as ModelExtra;
        ...
        {
            bones.Clear();
            foreach (ModelBone bone in Modèle.Bones)
            {
            ...
            foreach (ModelMesh modelMesh in Modèle.Meshes)
            {
                foreach (Effect effect in modelMesh.Effects)
                {
                    if (effect is BasicEffect)
                    {
                        ...
                        beffect.World = boneTransforms[modelMesh.ParentBone.Index] * this.GetMonde();
                        beffect.View = this.CaméraJeu.Vue;
                        beffect.Projection = this.CaméraJeu.Projection;
                    ...
                    if (effect is SkinnedEffect)
                    {
                        ...
                        seffect.World = boneTransforms[modelMesh.ParentBone.Index] * this.GetMonde();
                        seffect.View = this.CaméraJeu.Vue;
                        seffect.Projection = this.CaméraJeu.Projection;
